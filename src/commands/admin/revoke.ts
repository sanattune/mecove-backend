...
export async function handleRevoke({ messageText }: CommandContext): Promise<CommandResult> {
  ...
  if (!targetIdentity.user.approvedAt) return { kind: "reply", text: `${normalizedPhone} is not currently approved.` };
  await prisma.user.update({
    where: { id: targetIdentity.userId },
    data: { approvedAt: null },
  });
  return { kind: "reply", text: `${normalizedPhone} revoked.` };
}
