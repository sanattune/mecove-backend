...
*Notes*
Admin commands are not visible to regular users in /help or /guide. The /userstats output sorts by most recent activity first and shows the WhatsApp profile name when available, then the configured name, then the phone number. Revoking a user sets approvedAt to null - they will receive the waitlist message on their next message.
